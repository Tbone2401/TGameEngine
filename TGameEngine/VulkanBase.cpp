...
void VulkanBase::pickPhysicalDevice()
{
	...
	if (vkEnumeratePhysicalDevices(vInstance, &deviceCount, nullptr) != VK_SUCCESS || (deviceCount == 0))
	{
		throw std::runtime_error("failed to find GPUs with Vulkan support!");
	}

	std::vector<VkPhysicalDevice> devices(deviceCount);
	if (vkEnumeratePhysicalDevices(vInstance, &deviceCount, devices.data()) != VK_SUCCESS)
	{
		throw std::runtime_error("failed to find GPUs with Vulkan support!");
	}
...
void VulkanBase::createSwapChain()
{
	...
	uint32_t imageCount = swapChainSupport.capabilities.minImageCount + 1; //triple buffering
	...
	swapchainInfo.imageArrayLayers = 1; // always one (except for stereoscopic 3d)
	swapchainInfo.imageUsage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT; //can be different later when using post processing
